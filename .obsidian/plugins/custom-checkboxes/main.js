  ...
  async onload() {
    ...
    this.registerMarkdownPostProcessor((element, context) => {
      ...
      for (const taskItem of taskItems) {
        ...
        if (!checkbox)
          continue;
        // Mirror the data-task marker on the input for CSS styling
        try {
          if (char)
            checkbox.setAttribute("data-task", char);
        } catch (e) {
        }
        switch (char) {
          case ".":
          ...
          case "~":
            ...
            break;
          case ">":
            // Custom loading state: keep as custom box until clicked, then mark done
            registerLoadingClick(checkbox);
            break;
        }
      }
    });
    ...
    const registerLoadingClick = (checkbox) => {
      let handled = false;
      this.registerDomEvent(checkbox, "click", (evt) => {
        if (handled) {
          return;
